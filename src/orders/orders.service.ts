...
export class OrdersService {
  ...
  async create(createOrderDto: CreateOrderDto) {
    ...
    if (table.status !== 'AVAILABLE') {
      throw new BadRequestException(
        `Bàn ID ${tableId} hiện không trống (Đang ở trạng thái ${table.status})!`,
      );
    }
    ...
    if (reservationId) {
      ...
      if (!reservation) {
        throw new NotFoundException(
          `Đơn đặt bàn ID ${reservationId} không tồn tại!`,
        );
      }
    ...
    for (const item of items) {
      ...
      if (!menuItem) {
        throw new NotFoundException(
          `Món ăn ID ${item.menuItemId} không tồn tại!`,
        );
      }
    ...
    const orderItemsPromises = validMenuItems.map((item) => {
      ...
      if (
        item.dbData.is_flash_sale === true &&
        item.dbData.sale_price !== null
      ) {
    ...
    await this.diningTableRepository.update(tableId, {
    ...
    return {
      message:
        'Tạo đơn hàng thành công và đã khóa bàn sang trạng thái OCCUPIED!',
      order: savedOrder,
      items: orderItems,
    };
  }
  ...
  async updateItemStatus(itemId: number, updateDto: UpdateOrderItemStatusDto) {
    ...
    if (status === OrderItemStatus.DONE) {
      ...
      const activeItems = allItemsInOrder.filter(
        (item) => item.status !== 'CANCELLED',
      );

      const isAllActiveDone =
        activeItems.length > 0 &&
        activeItems.every((item) => item.status === 'DONE');
  ...
  async updateItemQuantity(
    ...
    if (order.status === 'BILLED' || order.status === 'PAID') {
      throw new BadRequestException(
        `Đơn hàng đã chốt hóa đơn hoặc thanh toán, không thể sửa số lượng!`,
      );
    }
    ...
    if (!orderItem)
      throw new NotFoundException(
        `Không tìm thấy dòng món ăn #${itemId} trong đơn hàng này!`,
      );
  ...
  async cancelOrderItem(orderId: number, itemId: number) {
    ...
    if (order.status === 'BILLED' || order.status === 'PAID') {
      throw new BadRequestException(
        `Đơn hàng đã chốt bill hoặc thanh toán, không thể hủy món!`,
      );
    }
    ...
    if (orderItem.status === 'CANCELLED') {
      throw new BadRequestException(
        `Món ăn này vốn đã ở trạng thái hủy từ trước!`,
      );
    }
    ...
    if (activeItems.length === 0) {
      ...
      const isAllActiveDone = activeItems.every(
