  ...
  async create(createDto: CreateReservationDto) {
    ...
    if (table.status === 'OCCUPIED') {
      throw new BadRequestException(
        'Bàn này đang có người sử dụng, không thể đặt!',
      );
    }

    if (guestCount > table.capacity) {
      throw new BadRequestException(
        `Bàn không đủ sức chứa! Bàn này chỉ phục vụ tối đa ${table.capacity} người.`,
      );
    }
    ...
    if (conflictingReservation) {
      throw new BadRequestException(
        'Bàn này đã có người đặt trong khung giờ bạn chọn. Vui lòng chọn giờ khác hoặc bàn khác!',
      );
    }
